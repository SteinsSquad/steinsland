...
class ClanPromoteCommand extends CustomCommand {

  public function __construct() {
    parent::__construct('cpromote', 'clan', 'steinscore.clan.create');
  ...
  public function onCommand(CommandSender $sender, array $args): int {
    ...
    if (SteinsClan::$instance->getPlayerClan($player) !== $clanName) {
      $sender->sendLocalizedMessage('clan.cpromote-failed');
      return self::RESULT_SUCCESS;
    }
    if (!(SteinsClan::$instance->hasPlayerRole($sender, SteinsClan::ROLE_OWNER))) return self::RESULT_NO_RIGHTS;
    $sender->sendLocalizedMessage('clan.cpromote-success', ['player' => SteinsPlayer::getPlayerName($player)]);
    SteinsClan::$instance->setPlayerRole($player, SteinsClan::ROLE_OFFICER);
    return self::RESULT_SUCCESS;
  }
}